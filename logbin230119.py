...
def logbin(data, scale = 1., zeros = False):
    ...
    if scale > 1:
        ...
        for i in range(len(y)):
            ...
            #ystddv = np.std(count[binedges[i]:binedges[i+1]]/(binedges[i+1] - binedges[i]))
...
def logbin2(data, scale = 1., zeros = False):
    """
    logbin(data, scale = 1., zeros = False)
    ...
    if scale > 1:
        ...
        x = (binedges[:-1] * (binedges[1:]-1)) ** 0.5
        y = np.zeros_like(x)
        ystddv = np.zeros_like(x)

        count = count.astype('float')
        for i in range(len(y)):
            y[i] = np.sum(count[binedges[i]:binedges[i+1]]/(binedges[i+1] - binedges[i]))
            ystddv[i] = np.std(count[binedges[i]:binedges[i+1]]/(binedges[i+1] - binedges[i]))
            # print(binedges[i],binedges[i+1])
        # print(smax,jmax,binedges,x)
        # print(x,y)
    else:
