...
plt.show()
"""
x,y = nw.minetsimulator(4,1000,smooth = 100)
ytheo1 = nw.theominet(x,4)
plt.scatter(np.log(x),ytheo1,label = "T",s = 2)
plt.scatter(np.log(x),np.log(y), label = "sim", s = 2)
x,y = nw.minetsimulator(8,1000,smooth = 100)
ytheo1 = nw.theominet(x,8)
plt.scatter(np.log(x),ytheo1,label = "T",s = 2)
plt.scatter(np.log(x),np.log(y), label = "sim", s = 2)
plt.legend()
plt.show()